...
class CNN(object):
    ...
    def __init__(self, input_width, num_input_channels, num_channels, kernel_sizes, strides,
        ...
        for ic, oc, ks, stride in zip([num_input_channels] + num_channels[:-1], num_channels, kernel_sizes, strides):
            conv_layer = Conv1D(in_channel=ic, 
                                out_channel=oc,
                                kernel_size=ks, 
                                stride=stride,
            ...
            out_width = (out_width - ks) // stride + 1

        self.flatten = Flatten()
        out_feature = num_linear_neurons
        self.linear_layer = Linear(out_width * num_channels[-1], out_feature, weight_init_fn=linear_weight_init_fn, bias_init_fn=bias_init_fn)
    ...
    def forward(self, x):
        ...
        for i, (conv_layer, activation) in enumerate(zip(self.convolutional_layers, self.activations)):
            x = conv_layer(x)
            x = activation(x)
        x = self.flatten(x)
    ...
    def backward(self, labels):
        ...
        grad = self.criterion.derivative() / m

        ## Your code goes here -->
        # Iterate through each layer in reverse order
        # <---------------------
        gradYlin = self.flatten.backward(self.linear_layer.backward(grad))
        for conv_layer, activation in zip(self.convolutional_layers[::-1], self.activations[::-1]):
            gradY = gradYlin * activation.derivative()
            gradYlin = conv_layer.backward(gradY)
