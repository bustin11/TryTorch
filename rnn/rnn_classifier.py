...
#        import pdb
#        pdb.set_trace()
        print(x.shape) # (batch_size, sequence length, input size) = (5, 10, 40)
        print(hidden.shape) # (num_layers, batch_size, hidden size) = (2, 5, 32)

        # store hidden states at each time step, [(seq_len+1) * (num_layers, batch_size, hidden_size)]
        for i in range(seq_len):
            curr_hidden = []
            hl_prev = self.x[:,i,:] # get the one below
            for j in range(self.num_layers): # one unique cell per layer
                curr_cell = self.rnn[j]
                ht_prev = self.hiddens[i][j] # get the one on the left
                h_out = curr_cell(hl_prev, ht_prev)
                curr_hidden.append(h_out) # (num_layers, batch_size, hidden_size)
                hl_prev = h_out
            self.hiddens.append(np.array(curr_hidden))

        last_hidden = self.hiddens[-1][-1]
        ll = self.output_layer(last_hidden)
        return ll
...
#        pdb.set_trace()
        ...
        for i in range(seq_len-1,-1,-1):
            dh_bot = 0
            for j in range(self.num_layers-1,-1,-1): # one unique cell per layer
                curr_cell = self.rnn[j]
                hl_prev = self.x[:,i,:] # get the one below
                if j > 0:
                    hl_prev = self.hiddens[i+1][j-1]
                ht_curr = self.hiddens[i+1][j]
                ht_prev = self.hiddens[i][j] # get the one on the left
                dh_bot, dh_left = curr_cell.backward(dh[j] + dh_bot, ht_curr, hl_prev, ht_prev)
                dh[j] = dh_left


        # nomralize by batch_size
        dh_0 = dh / batch_size
        return dh_0
